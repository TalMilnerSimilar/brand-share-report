...
import { masterBrandData, categoryTotals } from '../data/masterData';
import { chartData } from '../data/chartData';
import { funnelChartData } from '../data/funnelChartData';
import Tooltip from './Tooltip';

// Function to calculate current values from master data
const calculateCurrentValues = (metricIdx: number, metricKeys: readonly string[], chartDataSource: any) => {
  ...
  // Map chart metric keys to master data keys
  const metricMapping: Record<string, keyof typeof masterBrandData[0]> = {
    productViews: 'productViewsShare',
    unitsSold: 'unitsSoldShare', 
    revenue: 'revenueShare',
    brandedSearchVolume: 'brandedSearchShare',
    searchVisibility: 'searchVisibilityShare',
    shareOfPaidClicks: 'paidClicksShare',
    shareOfTotalClicks: 'totalClicksShare'
  };
  
  const shareKey = metricMapping[currentMetricKey];
  
  // Get the appropriate total volume for this metric
  const totalMapping: Record<string, number> = {
    productViews: categoryTotals.productViews,
    unitsSold: categoryTotals.unitsSold,
    revenue: categoryTotals.revenue,
    brandedSearchVolume: categoryTotals.brandedSearch,
    searchVisibility: categoryTotals.searchVisibility,
    shareOfPaidClicks: categoryTotals.paidClicks,
    shareOfTotalClicks: categoryTotals.totalClicks
  };
  
  const totalVolume = totalMapping[currentMetricKey];
  const isRevenue = currentMetricKey === 'revenue';

  return masterBrandData.map(brand => {
    const sharePercentage = brand[shareKey] as number;
    const absoluteValue = (sharePercentage / 100) * totalVolume;
    ...
    if (absoluteValue >= 1000000) {
      formattedValue = `${prefix}${(absoluteValue / 1000000).toFixed(1)}M`;
    } else if (absoluteValue >= 1000) {
      formattedValue = `${prefix}${(absoluteValue / 1000).toFixed(1)}K`;
    } else {
      formattedValue = `${prefix}${Math.round(absoluteValue)}`;
    }

    const shareFormatted = `${sharePercentage.toFixed(1)}%`;

    // Use metric-specific change if available (for Nike's strategic narrative), otherwise use default
    let changeFormatted = brand.change;
    let isPositive = brand.isPositive;
    
    if (brand.metricChanges) {
      const metricChange = brand.metricChanges[currentMetricKey as keyof typeof brand.metricChanges];
      if (metricChange) {
        changeFormatted = metricChange.change;
        isPositive = metricChange.isPositive;
      }
    }

    // Helper functions to format values  
    const formatCurrency = (sharePercent: number, total: number): string => {
    ...
    const formatNumber = (sharePercent: number, total: number): string => {
      const value = (sharePercent / 100) * total;
      if (value >= 1000) {
    ...
    // Create the row data based on current metric
    let rowData: RowData;
    if (currentMetricKey === 'productViews') {
      rowData = {
        brand: brand.brand,
        ...
        productViews: formattedValue,
        share: shareFormatted,
        change: changeFormatted,
        isPositive: isPositive,
        isHighlighted: brand.isHighlighted,
      };
    } else if (currentMetricKey === 'unitsSold') {
      rowData = {
        brand: brand.brand,
        totalRevenue: formatCurrency(brand.revenueShare, categoryTotals.revenue),
        unitsSold: formattedValue,
    ...
    } else if (currentMetricKey === 'revenue') {
      rowData = {
        brand: brand.brand,
      ...
      // For funnel metrics, store the value in the correct field based on the metric
      const baseRowData = {
        brand: brand.brand,
        totalRevenue: '-',
        unitsSold: '-',
      ...
      // Add the specific funnel metric value
      switch (currentMetricKey) {
        case 'brandedSearchVolume':
          rowData = { ...baseRowData, brandedSearchVolume: formattedValue };
          break;
        case 'searchVisibility':
          rowData = { ...baseRowData, searchVisibility: formattedValue };
          break;
        case 'shareOfPaidClicks':
          rowData = { ...baseRowData, shareOfPaidClicks: formattedValue };
          break;
        case 'shareOfTotalClicks':
          rowData = { ...baseRowData, shareOfTotalClicks: formattedValue };
          break;
        default:
          rowData = baseRowData;
      }
...
const parseNumber = (val: string) => {
  ...
  // Clean the value by removing all non-numeric characters except decimal point, minus sign, and plus sign
  let cleanVal = val.replace(/[$KMPP%\s]/g, '');
  
  // Handle different unit suffixes
  if (val.includes('M')) {
    const numPart = parseFloat(cleanVal);
  ...
  // For percentages and PP (percentage points), just return the numeric value
  if (val.includes('%') || val.includes('PP')) {
    const numPart = parseFloat(cleanVal);
    return isNaN(numPart) ? 0 : numPart;
  }
  
  // For regular numbers (including those with + or - prefix)
  const numPart = parseFloat(cleanVal);
  return isNaN(numPart) ? 0 : numPart;
};
    ...
    const sortedOthers = sortKey
      ? [...others].sort((a, b) => {
          ...
          return sortAsc ? aVal - bVal : bVal - aVal;
              ...
              : // Default sort by share (descending) - highest market share first
        [...others].sort((a, b) => {
          const aVal = parseNumber(a.share);
          const bVal = parseNumber(b.share);
          return bVal - aVal; // Descending order (highest share first)
        });
